...
* Main Page 
	...
	* Fix navigation at 1000px [x] 
	* Contact Page Link to Form at Bottom (button to redirect to contact form) [x] 
	* Make history one text size. [x] 
	* Fix Nav Links to sit in line [x] 

* Admin Login
	* Add route 
	* Add form 
	* Upon logging, set with Admin privledges 
	* Remove that link (admins login through a different portal) [x] 

* Admin Pages
	* View users model information [x]  
	* Delete and edit users  

* User Pages
	* Add user contact form [x] 
	* Submitting the form will enter info into uses model [x] 
